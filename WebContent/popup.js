...
function createPopup(name, link, title){
	...
	width = 300;
	top =  5 + (30 * position);
	left =  screen.width - (345 + (40 * position));		
	var aux = new popUp(left, top, width, height, name, link, "white", "#00385c", "16pt serif", title, "#00385c", "white", "lightgrey", "#00568c", "black", true, true, true, true, true, false,'img/ar_max.gif','img/ar_min.gif','img/ar_close.gif','img/resize.gif');
...
function moveAllPopus(id){
	hidebox(id);
/*
	var id_aux;
	var aux;
	var position_found;
	var altura,largura,height,width,top,left;
	var last=cidlist.length-1;
	...
	for(i=position_found+1;i<cidlist.length;i++){
		aux=cidlist[i];  
		cidlist[i-1]=aux;

		top =  5 + (30 * (i-1));
		left =  screen.width - (345 + (40 * (i-1)));		
		
		movePopup(aux,left,top);
	}
	cidlist[last]=id;
	top =  5 + (30 * (last));
	left =  screen.width - (345 + (40 * (last)));			
...
function ishidebox(id){
if(w3c){

if(d.getElementById(id+'_b').style.display == "none"){
	return true;
}else{
	return false;
}
...
if(w3c){
//var wMAX=d.gEl(id+'_max');
//wMAX.click();


d.getElementById(id+"_b").style.height=330+'px';
d.getElementById(id+"_s").style.height=(ie5)? 330+'px':330+5+'px';
d.getElementById(id+"_c").style.display='block';
d.getElementById(id+"_rs").style.display='block';
ns6bugfix();
...
d.src="javascript:MessageWin.window.location.reload();";
...
wCLS.onclick=new Function("moveAllPopus('"+cid+"');");
